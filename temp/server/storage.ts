...
function splitHourlyPrice(hourPrice: number): [number, number] {
  const firstHalfPrice = Math.floor(hourPrice / 2);
  return [firstHalfPrice, hourPrice - firstHalfPrice];
}
...
function generateTimeSlots(turfId: string, date: string, basePrice: number, weekendSurcharge = 0): TimeSlot[] {
  ...
  const addHour = (time: string, period: string, hourPrice: number, index: number) => {
    const startMinute = timeToMinutes(time);
    const [firstHalfPrice, secondHalfPrice] = splitHourlyPrice(hourPrice);
    slots.push({
      id: `${turfId}-${date}-${period}-${index}-first-half`,
      turfId,
      date,
      startTime: minutesToTime(startMinute),
      endTime: minutesToTime(startMinute + 30),
    ...
    slots.push({
      id: `${turfId}-${date}-${period}-${index}-second-half`,
      turfId,
      date,
      startTime: minutesToTime(startMinute + 30),
  ...
  ["06:00", "07:00", "08:00", "09:00", "10:00", "11:00"].forEach((time, index) => {
    addHour(time, "morning", basePrice + surcharge, index);
  });

  ["12:00", "13:00", "14:00", "15:00", "16:00", "17:00"].forEach((time, index) => {
    addHour(time, "afternoon", Math.round(basePrice * 1.2) + surcharge, index);
  });

  ["18:00", "19:00", "20:00", "21:00", "22:00"].forEach((time, index) => {
    addHour(time, "evening", Math.round(basePrice * 1.5) + surcharge, index);
  });
...
export class MemStorage implements IStorage {
  ...
  private async splitOpenHourlySlots(turfId: string, date: string): Promise<void> {
    const slots = Array.from(this.timeSlots.values()).filter((slot) => slot.turfId === turfId && slot.date === date);
    const byStartTime = new Map(slots.map((slot) => [slot.startTime, slot]));

    for (const slot of slots) {
      const startMinute = timeToMinutes(slot.startTime);
      const endMinute = timeToMinutes(slot.endTime);
      if (endMinute - startMinute !== 60) continue;

      const secondHalfStart = minutesToTime(startMinute + 30);
      const existingSecondHalf = byStartTime.get(secondHalfStart);

      const [firstHalfPrice, secondHalfPrice] = splitHourlyPrice(slot.price);
      const firstHalfSlot: TimeSlot = {
        ...slot,
        endTime: secondHalfStart,
        price: firstHalfPrice,
      };
      const secondHalfSlot: TimeSlot = {
        ...(existingSecondHalf || slot),
        id: existingSecondHalf?.id || `${slot.id}-second-half`,
        startTime: secondHalfStart,
        endTime: slot.endTime,
        price: secondHalfPrice,
        isBooked: slot.isBooked || existingSecondHalf?.isBooked || false,
        isBlocked: slot.isBlocked || existingSecondHalf?.isBlocked || false,
      };

      this.timeSlots.set(firstHalfSlot.id, firstHalfSlot);
      this.timeSlots.set(secondHalfSlot.id, secondHalfSlot);
      byStartTime.set(firstHalfSlot.startTime, firstHalfSlot);
      byStartTime.set(secondHalfSlot.startTime, secondHalfSlot);

      if (this.isDatabaseEnabled && pool) {
        await pool.query(
          `UPDATE time_slots
           SET end_time = $1, price = $2
           WHERE id = $3`,
          [firstHalfSlot.endTime, firstHalfSlot.price, firstHalfSlot.id],
        );
        if (existingSecondHalf) {
          await pool.query(
            `UPDATE time_slots
             SET end_time = $1, price = $2, is_booked = $3, is_blocked = $4
             WHERE id = $5`,
            [secondHalfSlot.endTime, secondHalfSlot.price, secondHalfSlot.isBooked, secondHalfSlot.isBlocked, secondHalfSlot.id],
          );
        } else {
          await pool.query(
            `INSERT INTO time_slots (id, turf_id, date, start_time, end_time, price, period, is_booked, is_blocked)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
             ON CONFLICT (turf_id, date, start_time) DO NOTHING`,
            [
              secondHalfSlot.id,
              secondHalfSlot.turfId,
              secondHalfSlot.date,
              secondHalfSlot.startTime,
              secondHalfSlot.endTime,
              secondHalfSlot.price,
              secondHalfSlot.period,
              secondHalfSlot.isBooked,
              secondHalfSlot.isBlocked,
            ],
          );
        }
      }
  ...
  private async syncBookedSlotsFromBookings(turfId: string, date: string): Promise<void> {
    const activeBookings = Array.from(this.bookings.values()).filter((booking) =>
      booking.turfId === turfId && booking.date === date && booking.status !== "cancelled"
    );
    const dateSlots = Array.from(this.timeSlots.values()).filter((slot) => slot.turfId === turfId && slot.date === date);
    const updates: TimeSlot[] = [];

    for (const slot of dateSlots) {
      const slotStartMinute = timeToMinutes(slot.startTime);
      const shouldBeBooked = activeBookings.some((booking) =>
        slotStartMinute >= timeToMinutes(booking.startTime) && slotStartMinute < timeToMinutes(booking.endTime)
      );
      if (slot.isBooked === shouldBeBooked) continue;

      const updated = { ...slot, isBooked: shouldBeBooked };
      this.timeSlots.set(slot.id, updated);
      updates.push(updated);
    }

    if (updates.length > 0 && this.isDatabaseEnabled && pool) {
      await Promise.all(updates.map((slot) =>
        pool!.query(
          `UPDATE time_slots SET is_booked = $1 WHERE id = $2`,
          [slot.isBooked, slot.id],
        )
      ));
    }
  }
  ...
  async getTimeSlots(turfId: string, date: string): Promise<TimeSlot[]> {
    ...
    await this.splitOpenHourlySlots(turfId, date);
    await this.syncBookedSlotsFromBookings(turfId, date);
  ...
  private assertBookingMatchesSlots(insertBooking: InsertBooking, slots: TimeSlot[]): void {
    const durationSteps = insertBooking.duration / 30;
    if (!Number.isInteger(durationSteps) || durationSteps !== slots.length) {
      throw Object.assign(new Error("Selected slots do not match booking duration"), { status: 400 });
    }

    const sortedSlots = [...slots].sort((a, b) => a.startTime.localeCompare(b.startTime));
    const startMinute = timeToMinutes(insertBooking.startTime);
    const expectedStartTimes = Array.from({ length: durationSteps }, (_value, index) =>
  ...
  async cancelBooking(id: string): Promise<Booking | undefined> {
    ...
    for (let minute = startMinute; minute < startMinute + duration; minute += 30) {
      const slot = slots.find(s => s.startTime === minutesToTime(minute));
